...
export function PaymentSuccessPage() {
  ...
  const [summary, setSummary] = useState<PaymentSummary>({
    ...
    status: 'paid',
    reference: searchParams.get('reference') || null,
    checkoutSessionId: searchParams.get('session_id') || null,
    ...
    note: 'Synchronising the payment result with the backend retrieval record.',
  });
      ...
      if (!Number.isFinite(retrievalRequestId) || retrievalRequestId <= 0) {
        if (!ignore) {
          ...
          setSummary((current) => ({
            ...
            note: 'No retrieval request id was supplied, so this page is showing the payment result only.',
          }));
        }
        return;
      }
      ...
      try {
        ...
        if (!ignore) {
          ...
          setSummary({
            ...
            note: 'Payment status has been written back to the retrieval request.',
          });
          setErrorMessage('');
          setIsSyncing(false);
        }
        ...
        if (!ignore) {
          setErrorMessage(
            getApiStyleErrorMessage(error, 'Could not confirm the payment with the backend.'),
          );
          setSummary((current) => ({
            ...current,
            paidAt: new Date().toISOString(),
            note: 'The success screen loaded, but the backend status update still needs attention.',
          }));
          setIsSyncing(false);
        }
      }
  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-6 pb-10">
      <div className="rounded-[2.5rem] border border-emerald-100 bg-white p-8 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.06)]">
        <div className="mb-6 flex h-20 w-20 items-center justify-center rounded-[1.5rem] bg-emerald-100">
          <CheckCircle2 className="h-10 w-10 text-emerald-600" />
        </div>
        <h1 className="mb-3 text-3xl font-black tracking-tight text-slate-900">Payment Success</h1>
        <p className="max-w-2xl text-base font-medium leading-relaxed text-slate-500">
          {isSyncing
            ? 'We are confirming the paid retrieval checkout with the backend now.'
            : 'The retrieval payment result page is now connected to the backend status update endpoint.'}
        </p>
      </div>
      ...
      <PaymentSummaryCard
        ...
        subtitle={
          isSyncing
            ? 'Writing the payment result back to /api/retrieval-requests/:id/payment-status.'
            : 'The retrieval payment is now represented by live backend data instead of a front-end placeholder.'
        }
      />
      ...
      <div className="flex flex-wrap gap-3">
        <Link
          ...
          className="inline-flex items-center gap-2 rounded-2xl bg-emerald-600 px-5 py-3 font-bold text-white hover:bg-emerald-500"
        >
          View Security Vault
        </Link>
        <Link
